...
Before going into details about BankSimulator, couple of words on Contracts:

[Checkout.TakeHomeChallenge.Contracts](src/Checkout.TakeHomeChallenge.Contracts) is a library, which define important
...
that extracts an API key from Headers and returns 401 for anything not matching hard-coded string. 
...
every 'failure' response should be standardized with ProblemDetails. 
...
1. POST /transactions will "transfer" money and return after transaction has been finalised. It will take 
between ~ 1 and 4 seconds. Response will contain the transaction id and it's status. 
...
json as input. It expects a minimal information and validates the format using System.ComponentModel.DataAnnotations.
...
Simulator is checked with [integration tests only](tests/Checkout.TakeHomeChallenge.BankSimulator.IntegrationTests)